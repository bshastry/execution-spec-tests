...
This directory contains example fuzzer outputs in both v2.0 and v3.0 JSON formats. These examples demonstrate the comprehensive transaction coverage generated by the fuzzer and serve as test cases for the fuzzer bridge implementation.

## Format Overview

### v2.0 Format (Transaction Distribution)

Most examples use fuzzer output format v2.0 with the following structure:
...
### v3.0 Format (Explicit Blocks)

**Feature Flag Required**: Set `FUZZER_BRIDGE_V3=true` to enable v3.0 support.

The v3.0 format explicitly defines blocks with per-block environment parameters:
...
- ✅ Blocks are explicitly specified (not generated by distribution)
- ✅ Per-block environment parameters (timestamp, gasLimit, baseFeePerGas, etc.)
- ✅ Withdrawal support (EIP-4895 Shapella)
- ✅ Per-block beacon roots (EIP-4788)
- ❌ No `env` field (replaced by per-block parameters)
- ❌ No `parentBeaconBlockRoot` at top level (per-block instead)

## Example Files

### fuzzer_test_v3_example.json (v3.0)
- **Format**: v3.0 (requires `FUZZER_BRIDGE_V3=true`)
- **Blocks**: 3 blocks (2 with transactions, 1 empty)
- **Features**: Per-block environments, timestamp progression
- **Use Case**: Demonstrating v3.0 format with explicit block structure
...
```bash
# Convert a single v2.0 example to blockchain test
...
For v3.0 format, enable the feature flag:

```bash
# Convert v3.0 example (automatic version detection)
FUZZER_BRIDGE_V3=true uv run python -m cli.fuzzer_bridge.cli \
    --input src/cli/fuzzer_bridge/examples/fuzzer_test_v3_example.json \
    --output /tmp/v3_output

# The converter automatically routes to v3.0 handler based on version field
# No need to specify num_blocks, block_strategy, or block_time for v3.0
```

**Note**: The converter automatically detects the version from the `version` field and routes to the appropriate converter (v2.0 or v3.0).
